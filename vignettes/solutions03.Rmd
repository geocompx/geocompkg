...
14) What was the minimum, average and maximum state-level change in people living below the poverty line for each region? Bonus: What is the region with the minimum overall change in people living below the poverty line?

```{r}
us_pov_change_reg = us_pov_change %>%
  group_by(REGION) %>%
  summarize(min_state_pov_change = min(pov_change),
            mean_state_pov_change = mean(pov_change),
            max_state_pov_change = max(pov_change),
            region_pov_change = sum(pov_change))

# Bonus
us_pov_pct_change_reg %>%
  filter(region_pov_change == min(region_pov_change)) %>%
  pull(REGION) %>%
  as.character()
```

15) Create a raster from scratch with nine rows and columns and a resolution of 0.5 decimal degrees (WGS84).
...
17) Plot the histogram and the boxplot of the `data(dem, package = "spDataLarge")` raster. 
```{r}
...
18) Now attach also `data(ndvi, package = "spDataLarge")`. 
