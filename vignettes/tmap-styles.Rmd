...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, eval = T,
...
Histogram is added using `legend.hist = TRUE` in this and several next examples to show how the selected map style relates to the distribution of values. 
...
Additionaly, the default lables can be overwirtten using the `labels` argument.
...
The `"sd"` style calculates a value of standard deviation of a given variable, and next use this value as the break width.
...
The `"fisher"` style creates a groups with maximilized homogenity.^[https://www.tandfonline.com/doi/abs/10.1080/01621459.1958.10501479] 
...
Better alternative in this case is to use a common logarithm (the logarithm to base 10) scale.
