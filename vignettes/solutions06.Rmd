...
vignette: >
  %\VignetteIndexEntry{geocompr-solutions6}
...
There are several differences between the original and the transformed raster, including a different number of rows and columns (and subsequently cells), and projection units.
The most important difference of the bilinear interpolation is a conversion of integer values in the original raster to float values in the output raster.
This operation should be avoided because float values are not suitable to represent categorical values.

5) Create your own `proj4string`. 
