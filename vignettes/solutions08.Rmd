...
9) How would app design, deployment and project management decisions change as the scale of map deployment increases?

10) Update the code in `coffeeApp/app.R` so that instead of centering on Brazil the user can select which country to focus on:
    - Using `textInput()`
    - Using `selectInput()`

11) Reproduce this [Figure](https://geocompr.robinlovelace.net/adv-map.html#fig:tmshape) and the 1st and 6th panel of this [Figure](https://geocompr.robinlovelace.net/adv-map.html#fig:break-styles) as closely as possible using the **ggplot2** package.
```{r}
# fig 8.1 
ggplot() + 
...
12) Join `us_states` and `us_states_df` together and calculate a poverty rate for each state using the new dataset.
...
13) Visualize population growth in Africa. 
